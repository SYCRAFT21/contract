...
//Submitted to Etherscan.io on 2023-07-01

// Website : https://hunterprotocol.org
// Telegram : https://t.me/HunterProtocol
...
library Address {
    function isContract(address account) internal view returns (bool) {
        ...
        //solhint disabled next line no-inline-assembly
    ...
    function functionCallWithValue(address target, bytes memory data, uint256 value) internal returns (bytes memory) {
        return functionCallWithValue(target, data, value, "Address: low-level call value failed");
    }
    ...
    function _functionCallWithValue(address target, bytes memory data, uint256 weiValue, string memory errorMessage) internal returns (bytes memory) {
        ...
        //solhint-disabled-next-line avoid low-level calls
        ...
        if (success) {
            ...
            if(returndata.length > 0) {
                //The easienst way to bubble the revert reason by using memory via assembly
                //solhint-disable-next-line no inline-asssembly
                assembly {
...
interface IUniswapV2Router01 {
    ...
    function addLiquidity(
        ...
        address to,
        uint deadline
    ) external returns (uint amountA, uint amoutB, uint liquidity);
    ...
    function getAmountIn(uint aountOut, uint reserveIn, uint reserveOut) external pure returns (uint amountIn);
...
contract HunterProtocol is Context, IERC20, Ownable {
    ...
    string private _name = "Hunter Protocol";
    ...
    constructor() 
    {
        ...
        totalBuyFees = taxOnBuy + liquidityFeeOnBuy + marketingFeeOnBuy;
        totalSellFees = taxOnSell + liquidityFeeOnSell + marketingFeeOnSell;

        marketingWallet = 0xCC3DdD8E3B19EA92626BC51D4fc239c2BC6D4b50;
    ...
    function totalReflectionDeistributed() public view returns (uint256) {
        return _tFeeTotal;
    }
    ...
    function setBuyFee() private {
        if(_taxFee == taxOnBuy && _liquidityFee == liquidityFeeOnBuy && _marketingFee == marketingFeeOnBuy) return;
    ...
    function setSwapTokensAtAmount(uint256 newAmount) external onlyOwner {
        require(newAmount > totalSupply() / 1e5, "Swap Tokens amount must be greater than 0.001% of total supply");
    ...
    function updateMarketingWallet(address _marketingWallet) external onlyOwner {
        require(_marketingWallet != marketingWallet, "Marketing wallet is the same address");
